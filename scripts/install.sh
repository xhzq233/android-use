...
if [[ "${1:-}" == "--help" ]]; then
    echo "Install android-use to ~/.local/share/android-use with an entry in ~/.local/bin."
    echo "Re-run to update. Requires Git and Python 3.9+. Does not install ADB or a Skill."
...
echo "Install Skill: $entry skill install --project  (or --user)"
